...
def fig_temperature_field():
    ...
    fig.subplots_adjust(left=0.06, right=0.92, top=0.88, bottom=0.12)
...
def fig_lunar_vs_earth():
    ...
    rows.append([
        ...
        f"{m_r1_lunar['Bo']:.2e}",
        f"{m_s1_lunar['Bo']:.2e}",
        "—  reduced by g_M/g_E ≈ 0.165"
    ])
    rows.append([
        "Bo regime",
        "Marangoni-dominated",
        "Marangoni-dominated",
        "for both alloys, both gravities"
    ])
    # Render the table as text
    n_cols = 4
    col_x = [0.00, 0.21, 0.41, 0.62]
    for j, header in enumerate(rows[0]):
        ax2.text(col_x[j], 0.92, header, transform=ax2.transAxes,
    ...
    for i, row in enumerate(rows[1:]):
        ...
        for j, cell in enumerate(row):
            ax2.text(col_x[j], y, cell, transform=ax2.transAxes,
                      fontsize=9.0, ha="left", va="top",
                      fontweight="bold" if j == 0 else "normal",
                      color="#37474F")
    # Bottom commentary
    ax2.text(0.0, 0.04,
             "All four Bond numbers are ≪ 1, so surface tension dominates over\n"
             "buoyancy in both terrestrial and lunar LPBF for these alloys.\n"
             "The melt-pool free-surface dynamics are Marangoni-driven; lunar\n"
             "operation reduces the (already small) buoyant contribution by\n"
             "~6×, but does not move either composition across a regime\n"
             "boundary at the nominal LPBF parameters.",
             transform=ax2.transAxes, fontsize=8.4, color="#37474F",
